          ...
          <div class="col s3">
            <div class="card">
              <div class="card-content">
                <div class="row">
                  <div class="col s12">
                    <form action="{{ route('mazhabList.update',$mazhab->id) }}" method="POST" enctype="multipart/form-data">
                      @csrf
                      @method('PUT')
                      <div class="col s12 input-field">
                        <select class="select2 browser-default @error('mazhabType') is-invalid @enderror" id="mazhabType" name="mazhabType" required>
                          <option selected disabled>{{__('Select Mazhab Type')}}</option>
                          @foreach ($mazhabTypes as $mazhabType)
                          <option {{ $mazhab->mazhabType == $mazhabType->id ? 'selected':''}} value="{{ $mazhabType->id }}">{!! $mazhabType->mazhabType !!}</option>
                          @endforeach
                        </select>
                      ...
                      <div class="col s12 input-field">
                        <input id="mazhab" name="mazhab" type="text" class="validate @error('mazhab') is-invalid @enderror" value="{{ $mazhab->mazhabName }}" required 
